...
void i_esp::esp_init()
{
	...
	if (crosshair)
	{
		float center_x = ImGui::GetIO().DisplaySize.x / 2.f;
		float center_y = ImGui::GetIO().DisplaySize.y / 2.f;
		ImVec2 lineStart(center_x - 10, center_y);
		ImVec2 lineEnd(center_x + 10, center_y);
		ImVec2 lineStartVertical(center_x, center_y - 10);
		ImVec2 lineEndVertical(center_x, center_y + 10);
	...
	if (ReplayInterface)
	{
		...
		for (int i = 0U; i < entitylist; i++)
		{
			...
			if (Distance <= distanceint)
			{
				...
				if (lines)
				{
					ImGui::GetBackgroundDrawList()->AddLine(ImVec2(FiveM::WindowSize.x / 2, FiveM::WindowSize.y), ImVec2(Ped_Location.x, Ped_Location.y + Height / 2), LineColor, 1);
					// WindowSize
				}
				...
				if (heathbar)
				{
					...
					ImColor	col = ImColor(0, 255, 34);
					ImColor coll = ImColor(0, 255, 34);
				...
				if (armorbar)
				{
					...
					float armorped = *(float*)(Ped + FiveM::Armor); // wait 2sec
