...
#include "invoker_definitions.hpp"
...
typedef std::unordered_map<NativeHash, NativeHandler> g_fastPathMap_t;
inline g_fastPathMap_t g_fastPathMap;
inline g_fastPathMap_t* orin_fastPathMap;

typedef std::unordered_map<uint64_t, uint64_t> g_mappingTable_t;
inline std::unordered_map<uint64_t, uint64_t> g_mappingTable;
static std::vector<std::pair<uint64_t, NativeHandler>> g_nativeHandlers;

inline NativeHandler(__fastcall* GetNativeHandlerDo_Old)(__int64, __int64);
inline NativeHandler(__fastcall* GetNativeHandlerDo_Obfuscated)(__int64, __int64);

inline uint64_t MapNative(uint64_t inNative)
{
...
inline bool safeCheck(uint64_t addr)
{
	return *(BYTE*)addr == 0x4C;
}
inline NativeHandler GetNativeHandler(uint64_t hash)
{
	if (auto it = orin_fastPathMap->find(NativeHash{ hash }); it != orin_fastPathMap->end() && it->second)
	{
		return it->second;
	}

	uint64_t mappedNative = MapNative(hash);
	return (/*data::game_build == 2372 ? GetNativeHandlerDo_Old(hash, mappedNative) :*/ GetNativeHandlerDo_Obfuscated(hash, mappedNative));
	

}

inline NativeHandler GetNativeHandlerOrig(uint64_t hash)
{
...
//	g_nativeHandlers = *(std::vector<std::pair<uint64_t, NativeHandler>>*)(rage_base + 0x310338);
//	for (const auto& pair : g_nativeHandlers) {
//		MessageBoxA(NULL, std::to_string(pair.first).c_str(), "native bzr", NULL);
//	}
}
...
inline void nativePush64(UINT64 value)
{
...
{
	NativeHandler handler = GetNativeHandler(g_hash);
...
public:
	...
	PVector3(float x, float y, float z) :
		x(x), y(y), z(z)
	{}
	PVector3 operator + (const PVector3& rhs) const { return PVector3(x + rhs.x, y + rhs.y, z + rhs.z); }
	PVector3 operator - (const PVector3& rhs) const { return PVector3(x - rhs.x, y - rhs.y, z - rhs.z); }
	PVector3 operator * (const float& rhs) const { return PVector3(x * rhs, y * rhs, z * rhs); }
...
inline NativeHandler invisible(uint64_t hash) {
	return [](scrNativeCallContext* cxt) {
		Entity ped = cxt->GetArgument<Entity>(0);

		cxt->SetResult<bool>(0, true);
	};
}
inline NativeHandler MyCustomHandler(uint64_t hash) {
	return [](scrNativeCallContext* cxt) {
		Ped ped = cxt->GetArgument<Ped>(0);
		int maxHealth = 777;
		cxt->SetResult<int>(0, maxHealth);
	};
}
inline NativeHandler Decor(uint64_t hash) {
	return [](scrNativeCallContext* cxt) {

		Entity entity = cxt->GetArgument<Entity>(0);
		const char* propertyName = cxt->GetArgument<const char*>(1);


		bool result = true;

		cxt->SetResult<bool>(0, result);
	};
}
inline NativeHandler GET_SELECTED_WEAPON(uint64_t hash) {
	return [](scrNativeCallContext* cxt) {
		Ped ped = cxt->GetArgument<Ped>(0);
		cxt->SetResult<Hash>(0, 0xA2719263);
	};
}
inline NativeHandler WEAPONM(uint64_t hash) {
	return [](scrNativeCallContext* cxt) {
		Ped ped = cxt->GetArgument<Ped>(0);
		Hash weaponHash = cxt->GetArgument<Hash>(1);
		bool p2 = cxt->GetArgument<bool>(2);

		bool spoofed = false;
		cxt->SetResult<bool>(0, spoofed);
	};
}
inline NativeHandler GET_CURRENT_WEAPON(uint64_t hash) {
	return [](scrNativeCallContext* cxt) {
		Ped ped = cxt->GetArgument<Ped>(0);
		Hash weaponHash = cxt->GetArgument<Hash>(1);
		bool p2 = cxt->GetArgument<bool>(2);
		//    Entity entity = cxt->GetArgument<Entity>(3);
			// modify the arguments or perform any other logic
		bool spoofed = false;

		// set the result of the native call
		cxt->SetResult<bool>(0, spoofed);
	};
}
bool HasPrinted = false;
inline void PrintAddress(DWORD64 address) {
	std::stringstream hexStream;
	hexStream << std::hex << std::showbase << address;
	std::string hexString = hexStream.str();

}
char hashh[999] = "0x91310870";
char consolepush[999] = "Test";

std::vector<std::string> console_str;
std::vector<std::string> console_str2;
std::vector<std::string> console_str3;
std::vector<std::string> breakednative_str;
std::vector<std::string> console_str4;
inline void HookNattt(uint64_t hash, NativeHandler Handler)
{
	auto orig = GetNativeHandler(hash);
	if (orig) {
		NativeHandler modifiedHandler = Handler;
		g_fastPathMap[NativeHash{ hash }] = modifiedHandler;
	}
}

inline NativeHandler breakn(uint64_t hash) {
	return [](scrNativeCallContext* cxt) {
		cxt->SetResult<uintptr_t>(0, 0); \

	};
}
std::vector<uint64_t> breakednative;
bool filterbycfxnative = false;
inline void breaknative()
{
	for (int index = 0; index < static_cast<int>(breakednative.size()); index++)
	{
		HookNattt(breakednative[index], breakn(breakednative[index]));
	}
}
...
//
//	auto file = std::ofstream("C:\\Users\\ihaba\\AppData\\Local\\FiveM\\FiveM.app\\plugins\\caca.txt", std::ios::out | std::ios::trunc);
//
//	for (auto& map : g_fastPathMap)
//	{
//		auto address = GetNativeHandlerOrig((uint64_t)map.first.GetHash());
//
//		file << std::hex << std::uppercase << "0x" << map.second << " : GTA5.exe + 0x" << (DWORD64)address - base_address << std::endl;
//	}
//
//	file.close();
//}
inline void HookNat(uint64_t hash, NativeHandler Handler)
{
	NativeHandler originalHandler = GetNativeHandler(hash);
	if (originalHandler) {
		NativeHandler modifiedHandler = Handler;
		//(*orin_fastPathMap)[NativeHash{ hash }] = modifiedHandler;
		g_fastPathMap[NativeHash{ hash }] = modifiedHandler;
	}
}	

inline void HookNative1()
{
	//HookNat(0x3A87E44BB9A01D54, GET_CURRENT_WEAPON(0x3A87E44BB9A01D54));
	//HookNat(0x0A6DB4965674D243, GET_SELECTED_WEAPON(0x0A6DB4965674D243));
	//HookNat(0x47D6F43D77935C75, invisible(0x47D6F43D77935C75));
	//HookNat(0x8DECB02F88F428BC, WEAPONM(0x8DECB02F88F428BC));
	...
	if (!HasPrinted)
	{
		...
		//dump_entry_points();
		PrintAddress((DWORD64)GetNativeHandler(0x4700A416E8324EF3));
		//MessageBoxA(NULL,"Just dumped everything","Check C Disk",NULL);
		HasPrinted = true;
	}
...
void ScanMap(){
//	orin_fastPathMap =	Memory::PatternScanNewEx(ragge_base, E("48 8D 0D ? ? ? ? E8 ? ? ? ? 48 8B 08 48 89 59 ? 4C 8B 4C 24"), NULL, 7));
	g_fastPathMap = *(g_fastPathMap_t*)orin_fastPathMap;
}
namespace HUD
{
	...
	static void _0x9245E81072704B8A(BOOL p0) { invoke<Void>(0x9245E81072704B8A, p0); } // 0x9245E81072704B8A b323
	static void _SET_MOUSE_CURSOR_ACTIVE_THIS_FRAME() { invoke<Void>(0xAAE7CE1D63167423); } // 0xAAE7CE1D63167423 b323
	...
	static void _THEFEED_ADD_TXD_REF(Any* p0, Any* p1, Any* p2, Any* p3) { invoke<Void>(0x317EBA71D7543F52, p0, p1, p2, p3); } // 0x317EBA71D7543F52 b323
	...
	static int _END_TEXT_COMMAND_THEFEED_POST_REPLAY_ICON(int type, int image, const char* text) { return invoke<int>(0xD202B92CBF1D816F, type, image, text); } // 0xD202B92CBF1D816F b323
	static int _END_TEXT_COMMAND_THEFEED_POST_REPLAY_INPUT(int type, const char* button, const char* text) { return invoke<int>(0xDD6CB2CCE7C2735C, type, button, text); } // 0xDD6CB2CCE7C2735C b323
	...
	static void _BEGIN_TEXT_COMMAND_GET_WIDTH(const char* text) { invoke<Void>(0x54CE8AC98E120CAB, text); } // 0x54CE8AC98E120CAB 0x51E7A037 b323
	...
	static void _BEGIN_TEXT_COMMAND_LINE_COUNT(const char* entry) { invoke<Void>(0x521FB041D93DD0E4, entry); } // 0x521FB041D93DD0E4 0x94B82066 b323
	...
	static const char* _0x98C3CF913D895111(const char* string, int length) { return invoke<const char*>(0x98C3CF913D895111, string, length); } // 0x98C3CF913D895111 b505
	...
	static void _0xCD74233600C4EA6B(BOOL toggle) { invoke<Void>(0xCD74233600C4EA6B, toggle); } // 0xCD74233600C4EA6B b505
	static BOOL _0xC2D2AD9EAAE265B8() { return invoke<BOOL>(0xC2D2AD9EAAE265B8); } // 0xC2D2AD9EAAE265B8 b505
	...
	static void _0x0C698D8F099174C7(Any p0) { invoke<Void>(0x0C698D8F099174C7, p0); } // 0x0C698D8F099174C7 b1180
	static void _0xE4C3B169876D33D7(Any p0) { invoke<Void>(0xE4C3B169876D33D7, p0); } // 0xE4C3B169876D33D7 b1290
	static void _0xEB81A3DADD503187() { invoke<Void>(0xEB81A3DADD503187); } // 0xEB81A3DADD503187 b1290
	...
	static void _CLEAR_ALL_BLIP_ROUTES() { invoke<Void>(0xD12882D3FF82BF11); } // 0xD12882D3FF82BF11 b877
	...
	static void _0x2790F4B17D098E26(BOOL toggle) { invoke<Void>(0x2790F4B17D098E26, toggle); } // 0x2790F4B17D098E26 b573
	static void _0x6CDD58146A436083(Any p0) { invoke<Void>(0x6CDD58146A436083, p0); } // 0x6CDD58146A436083 b573
	static void _0xD1942374085C8469(BOOL disabled) { invoke<Void>(0xD1942374085C8469, disabled); } // 0xD1942374085C8469 b505
	...
	static void _0x57D760D55F54E071(int p0) { invoke<Void>(0x57D760D55F54E071, p0); } // 0x57D760D55F54E071 0x9854485F b323
	...
	static void _0xD2049635DEB9C375() { invoke<Void>(0xD2049635DEB9C375); } // 0xD2049635DEB9C375 0xE8D3A910 b323
	...
	static void _SET_SCRIPT_VARIABLE_2_HUD_COLOUR(int r, int g, int b, int a) { invoke<Void>(0x16A304E6CB2BFAB9, r, g, b, a); } // 0x16A304E6CB2BFAB9 0x6BE3ACA8 b323
	...
	static PVector3 GET_BLIP_INFO_ID_COORD(Blip blip) { return invoke<PVector3>(0xFA7C7F0AADF25D09, blip); } // 0xFA7C7F0AADF25D09 0xB7374A66 b323
	...
	static PVector3 GET_BLIP_COORDS(Blip blip) { return invoke<PVector3>(0x586AFE3FF72D996E, blip); } // 0x586AFE3FF72D996E 0xEF6FF47B b323
	...
	static void _0x9FCB3CBFB3EAD69A(int p0, float p1) { invoke<Void>(0x9FCB3CBFB3EAD69A, p0, p1); } // 0x9FCB3CBFB3EAD69A b1734
	static void _0xB7B873520C84C118() { invoke<Void>(0xB7B873520C84C118); } // 0xB7B873520C84C118 b1734
	...
	static int _GET_BLIP_ROTATION(Blip blip) { return invoke<int>(0x003E92BA477F9D7F, blip); } // 0x003E92BA477F9D7F b2060
	...
	static void _0x2916A928514C9827() { invoke<Void>(0x2916A928514C9827); } // 0x2916A928514C9827 b573
	static void _0xB552929B85FC27EC(Any p0, Any p1) { invoke<Void>(0xB552929B85FC27EC, p0, p1); } // 0xB552929B85FC27EC b573
	...
	static void SET_BLIP_AS_MINIMAL_ON_EDGE(Blip blip, BOOL toggle) { invoke<Void>(0x2B6D467DAB714E8D, blip, toggle); } // 0x2B6D467DAB714E8D 0xC575F0BC b323
	...
	static void _DELETE_WAYPOINT() { invoke<Void>(0xD8E694757BCEA8E9); } // 0xD8E694757BCEA8E9 0x62BABF2C b323
	...
	static void _SET_INTERIOR_ZOOM_LEVEL_INCREASED(BOOL toggle) { invoke<Void>(0x504DFE62A1692296, toggle); } // 0x504DFE62A1692296 b1493
	static void _SET_INTERIOR_ZOOM_LEVEL_DECREASED(BOOL toggle) { invoke<Void>(0x7EC8ABA5E74B3D7A, toggle); } // 0x7EC8ABA5E74B3D7A b2372
	static void SET_RADAR_AS_EXTERIOR_THIS_FRAME() { invoke<Void>(0xE81B7D2A3DAB2D81); } // 0xE81B7D2A3DAB2D81 0x39ABB10E b323
	static void _SET_PLAYER_BLIP_POSITION_THIS_FRAME(float x, float y) { invoke<Void>(0x77E2DD177910E1CF, x, y); } // 0x77E2DD177910E1CF 0x54E75C7D b323
	static void _0xA17784FCA9548D15(Any p0, Any p1, Any p2) { invoke<Void>(0xA17784FCA9548D15, p0, p1, p2); } // 0xA17784FCA9548D15 b877
	...
	static void _0x170F541E1CADD1DE(BOOL p0) { invoke<Void>(0x170F541E1CADD1DE, p0); } // 0x170F541E1CADD1DE b323
	static void _SET_PLAYER_CASH_CHANGE(int cash, int bank) { invoke<Void>(0x0772DF77852C2E30, cash, bank); } // 0x0772DF77852C2E30 b323
	...
	static void _0xE67C6DFD386EA5E7(BOOL p0) { invoke<Void>(0xE67C6DFD386EA5E7, p0); } // 0xE67C6DFD386EA5E7 0x5476B9FD b323
	...
	static BOOL _0x801879A9B4F4B2FB() { return invoke<BOOL>(0x801879A9B4F4B2FB); } // 0x801879A9B4F4B2FB b372
	...
	static void _SET_MAIN_PLAYER_BLIP_COLOUR(int color) { invoke<Void>(0x7B21E0BB01E8224A, color); } // 0x7B21E0BB01E8224A b323
	...
	static void _SET_MISSION_NAME_2(BOOL p0, const char* name) { invoke<Void>(0xE45087D85F468BC2, p0, name); } // 0xE45087D85F468BC2 0x8D9A1734 b323
	static void _0x817B86108EB94E51(BOOL p0, Any* p1, Any* p2, Any* p3, Any* p4, Any* p5, Any* p6, Any* p7, Any* p8) { invoke<Void>(0x817B86108EB94E51, p0, p1, p2, p3, p4, p5, p6, p7, p8); } // 0x817B86108EB94E51 0xD2161E77 b323
	...
	static void _HIDE_AREA_AND_VEHICLE_NAME_THIS_FRAME() { invoke<Void>(0xA4DEDE28B1814289); } // 0xA4DEDE28B1814289 0x52746FE1 b323
	...
	static PVector3 GET_HUD_COMPONENT_POSITION(int id) { return invoke<PVector3>(0x223CA69A8C4417FD, id); } // 0x223CA69A8C4417FD 0x080DCED6 b323
	...
	static void _SET_MP_GAMER_TAG_UNK(int gamerTagId, int p1) { invoke<Void>(0x9C16459B2324B2CF, gamerTagId, p1); } // 0x9C16459B2324B2CF b877
	...
	static BOOL _IS_VALID_MP_GAMER_TAG_MOVIE(int gamerTagId) { return invoke<BOOL>(0xEB709A36958ABE0D, gamerTagId); } // 0xEB709A36958ABE0D 0xF11414C4 b323
	...
	static void SET_WARNING_MESSAGE(const char* titleMsg, int flags, const char* promptMsg, BOOL p3, int p4, const char* p5, const char* p6, BOOL showBackground, int errorCode) { invoke<Void>(0x7B1776B3B53F8D74, titleMsg, flags, promptMsg, p3, p4, p5, p6, showBackground, errorCode); } // 0x7B1776B3B53F8D74 0xBE699BDE b323
	...
	static void SET_WARNING_MESSAGE_WITH_HEADER_AND_SUBSTRING_FLAGS(const char* entryHeader, const char* entryLine1, int instructionalKey, const char* entryLine2, BOOL p4, Any p5, Any additionalIntInfo, const char* additionalTextInfoLine1, const char* additionalTextInfoLine2, BOOL showBackground, int errorCode) { invoke<Void>(0x701919482C74B5AB, entryHeader, entryLine1, instructionalKey, entryLine2, p4, p5, additionalIntInfo, additionalTextInfoLine1, additionalTextInfoLine2, showBackground, errorCode); } // 0x701919482C74B5AB 0x749929D3 b323
	...
	static void CUSTOM_MINIMAP_SET_ACTIVE(BOOL toggle) { invoke<Void>(0x5354C5BA2EA868A4, toggle); } // 0x5354C5BA2EA868A4 0xE4FD20D8 b323
	static void CUSTOM_MINIMAP_SET_BLIP_OBJECT(int spriteId) { invoke<Void>(0x1EAE6DD17B7A5EFA, spriteId); } // 0x1EAE6DD17B7A5EFA 0x13E7A5A9 b323
	static Any CUSTOM_MINIMAP_CREATE_BLIP(float x, float y, float z) { return invoke<Any>(0x551DF99658DB6EE8, x, y, z); } // 0x551DF99658DB6EE8 0x786CA0A2 b323
	static void CUSTOM_MINIMAP_CLEAR_BLIPS() { invoke<Void>(0x2708FC083123F9FF); } // 0x2708FC083123F9FF 0xCBEC9369 b323
	...
	static PVector3 _0x5BFF36D6ED83E0AE() { return invoke<PVector3>(0x5BFF36D6ED83E0AE); } // 0x5BFF36D6ED83E0AE b323
	static BOOL IS_PAUSE_MENU_RESTARTING() { return invoke<BOOL>(0x1C491717107431C7); } // 0x1C491717107431C7 0x3C4CF4D9 b323
	static void _LOG_DEBUG_INFO(const char* p0) { invoke<Void>(0x2162C446DFDF38FD, p0); } // 0x2162C446DFDF38FD 0x2DFD35C7 b323
	...
	static BOOL PAUSE_MENU_IS_CONTEXT_MENU_ACTIVE() { return invoke<BOOL>(0x2A25ADC48F87841F); } // 0x2A25ADC48F87841F 0x016D7AF9 b323
	static int _0xDE03620F8703A9DF() { return invoke<int>(0xDE03620F8703A9DF); } // 0xDE03620F8703A9DF b323
	static int _0x359AF31A4B52F5ED() { return invoke<int>(0x359AF31A4B52F5ED); } // 0x359AF31A4B52F5ED b323
	static int _0x13C4B962653A5280() { return invoke<int>(0x13C4B962653A5280); } // 0x13C4B962653A5280 b323
	...
	static void PAUSE_MENU_SET_BUSY_SPINNER(BOOL p0, int position, int spinnerIndex) { invoke<Void>(0xC78E239AC5B2DDB9, p0, position, spinnerIndex); } // 0xC78E239AC5B2DDB9 0x6C67131A b323
	...
	static BOOL _0x66E7CB63C97B7D20() { return invoke<BOOL>(0x66E7CB63C97B7D20); } // 0x66E7CB63C97B7D20 0x92DAFA78 b323
	static int _0x593FEAE1F73392D4() { return invoke<int>(0x593FEAE1F73392D4); } // 0x593FEAE1F73392D4 0x22CA9F2A b323
	static BOOL IS_NAVIGATING_MENU_CONTENT() { return invoke<BOOL>(0x4E3CD0EF8A489541); } // 0x4E3CD0EF8A489541 0xDA7951A2 b323
	static BOOL _0xF284AC67940C6812() { return invoke<BOOL>(0xF284AC67940C6812); } // 0xF284AC67940C6812 0x7D95AFFF b323
	...
	static BOOL _0xA238192F33110615(int* r, int* g, int* b) { return invoke<BOOL>(0xA238192F33110615, r, g, b); } // 0xA238192F33110615 0x46794EB2 b323
	...
	static BOOL GET_MENU_PED_MASKED_INT_STAT(Hash p0, Any* p1, Any p2, int p3) { return invoke<BOOL>(0x90A6526CF0381030, p0, p1, p2, p3); } // 0x90A6526CF0381030 0xD6CC4766 b323
	...
	static void _SET_PED_HAS_AI_BLIP_WITH_COLOR(Ped ped, BOOL hasCone, int color) { invoke<Void>(0xB13DCB4C6FAAD238, ped, hasCone, color); } // 0xB13DCB4C6FAAD238 b505
	...
	static void _SET_PED_AI_BLIP_SPRITE(Ped ped, int spriteId) { invoke<Void>(0xFCFACD0DB9D7A57D, ped, spriteId); } // 0xFCFACD0DB9D7A57D b877
	...
	static BOOL _HAS_DIRECTOR_MODE_BEEN_TRIGGERED() { return invoke<BOOL>(0xA277800A9EAE340E); } // 0xA277800A9EAE340E b323
	static void _SET_DIRECTOR_MODE_CLEAR_TRIGGERED_FLAG() { invoke<Void>(0x2632482FD6B9AB87); } // 0x2632482FD6B9AB87 b323
...
namespace ENTITY
{
	...
	static PVector3 GET_ENTITY_COORDS(Entity entity, BOOL p1) { return invoke<PVector3>(0x3FEF770D40960D5A, entity, p1); } // 0x3FEF770D40960D5A 0x1647F1CB
	static PVector3 GET_ENTITY_FORWARD_VECTOR(Entity entity) { return invoke<PVector3>(0x0A794A5A57F8DF91, entity); } // 0x0A794A5A57F8DF91 0x84DCECBF
	...
	static PVector3 GET_OFFSET_FROM_ENTITY_IN_WORLD_COORDS(Entity entity, float xOffset, float yOffset, float zOffset) { return invoke<PVector3>(0x1899F328B0E12848, entity, xOffset, yOffset, zOffset); } // 0x1899F328B0E12848 0xABCF043A
	...
	static PVector3 GET_ENTITY_ROTATION(Entity entity, int p1) { return invoke<PVector3>(0xAFBD61CC738D9EB9, entity, p1); } // 0xAFBD61CC738D9EB9 0x8FF45B04
...
namespace AI
{
	static void _CLEAR_VEHICLE_TASKS(Any veh) { invoke<Void>(0xDBBC7A2432524127, veh); } // 0xDBBC7A2432524127
...
namespace GAMEPLAY
{
	...
	static void GET_MODEL_DIMENSIONS(Hash modelHash, PVector3* minimum, PVector3* maximum) { invoke<Void>(0x03E8D3D5F549087A, modelHash, minimum, maximum); } // 0x03E8D3D5F549087A 0x91ABB8E0
...
namespace CAM
{
	...
	static PVector3 GET_CAM_COORD(Any cam) { return invoke<PVector3>(0xBAC038F7459AE5AE, cam); } // 0xBAC038F7459AE5AE 0x7C40F09C
	static PVector3 GET_CAM_ROT(Any cam, Any p1) { return invoke<PVector3>(0x7D304C1C955E3E12, cam, p1); } // 0x7D304C1C955E3E12 0xDAC84C9F
	...
	static PVector3 GET_GAMEPLAY_CAM_COORD() { return invoke<PVector3>(0x14D6F5678D8F1B37); } // 0x14D6F5678D8F1B37 0x9388CF79
	static PVector3 GET_GAMEPLAY_CAM_ROT(Any p0) { return invoke<PVector3>(0x837765A25378F0BB, p0); } // 0x837765A25378F0BB 0x13A010B5
...
namespace WEAPON
{
	...
	static void ShootSingleBulletBetweenCoords(float x1, float y1, float z1, float x2, float y2, float z2, int damage, bool pureAccuracy, DWORD weaponHash, Ped ownerPed, bool audiable, bool visible, float speed)
	{
		invoke<void*, float, float, float, float, float, float, int, bool, DWORD, Ped, bool, bool, float>(0x867654CBC7606F2C, x1, y1, z1, x2, y2, z2, damage, pureAccuracy, weaponHash, ownerPed, audiable, visible, speed);
	}
	...
	static BOOL REFILL_AMMO_INSTANTLY(Ped ped) { return invoke<BOOL>(0x8C0D57EA686FAD87, ped); } // 0x8C0D57EA686FAD87 0x82EEAF0F
...
namespace WORLDPROBE
{
	...
	static int GET_SHAPE_TEST_RESULT(int rayHandle, BOOL* hit, PVector3* endCoords, PVector3* surfaceNormal, Entity* entityHit) { return invoke<int>(0x3D87450E15D98694, rayHandle, hit, endCoords, surfaceNormal, entityHit); } // 0x3D87450E15D98694 0xF3C2875A
...
namespace NETWORK
{
	...
	static Player NETWORK_GET_PLAYER_INDEX_FROM_PED(Ped p0) { return invoke<Player>(0x6C0E2E0125610278, p0); } // 0x6C0E2E0125610278 0x40DBF464
	...
	static void NetworkRegisterEntityAsNetworked(Entity p0) { invoke<Void>(0x06FAACD625D80CAA, p0); } // 0x06FAACD625D80CAA 0x31A630A4
	...
	static void NETWORK_SET_ENTITY_INVISIBLE_TO_NETWORK(Any p0, BOOL p1) { invoke<Void>(0xF1CA12B18AEF5298, p0, p1); } // 0xF1CA12B18AEF5298 0x09CBC4B0
